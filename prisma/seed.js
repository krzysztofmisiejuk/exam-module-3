...
async function main() {
	...
	for (const category of allCategories) {
		for (let i = 0; i < allCategories.length; i++) {
			...
			const discount =
				i === 0 ? 59.99 + Number(Math.floor(Math.random() * 5).toFixed()) : null

			await prisma.product.create({
				data: {
					name: `${category.name} ${models[category.id - 1]}${
						(i + 1) * Number(Math.floor(Math.random() * 100).toFixed())
					}`,
					description: productDescriptions[i % productDescriptions.length],
					price: 89.99 + i + Number(Math.floor(Math.random() * 50).toFixed()),
					stock: 50,
